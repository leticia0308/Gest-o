...
<table class="table table-sm mt-3">
	<thead class="thead-dark">
		<tr>
			<th scope="col">Mural de Ocorrências</th>
	...
	if($txtbuscar == ''){
		$res = $pdo->query("SELECT * from home order by id desc LIMIT $limite, $itens_por_pagina");
	}else{
		$txtbuscar = '%'.@$_POST['txtbuscar'].'%';
		$res = $pdo->query("SELECT * from home where assunto LIKE '$txtbuscar' or assunto LIKE '$txtbuscar' order by id desc");
	...
	//TOTALIZAR OS REGISTROS PARA PAGINAÇÃO
		$res_todos = $pdo->query("SELECT * from home");
	...
	for ($i=0; $i < count($dados); $i++) { 
			...
			$status_ocorrencia = $dados[$i]['status_ocorrencia'];

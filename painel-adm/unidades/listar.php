...
<table class="table table-sm mt-3 tabelas">
	...
	if($txtbuscar == ''){
		$res = $pdo->query("SELECT * from unidades order by id desc LIMIT $limite, $itens_por_pagina");
	}else{
		$txtbuscar = '%'.@$_POST['txtbuscar'].'%';
		$res = $pdo->query("SELECT * from unidades where nome LIKE '$txtbuscar' or proprietario LIKE '$txtbuscar' or condominio LIKE '$txtbuscar' order by id desc");
	...
	//TOTALIZAR OS REGISTROS PARA PAGINAÇÃO
		$res_todos = $pdo->query("SELECT * from unidades");
	...
	for ($i=0; $i < count($dados); $i++) { 
			...
			$id = $dados[$i]['id'];	
			$nome = $dados[$i]['nome'];
			$identificacao = $dados[$i]['identificacao'];
			$proprietario = $dados[$i]['proprietario'];

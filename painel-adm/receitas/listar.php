...
<table class="table table-sm mt-3">
	...
	if($txtbuscar == ''){
		$res = $pdo->query("SELECT * from receitas order by id desc LIMIT $limite, $itens_por_pagina");
	}else{
		$txtbuscar = '%'.@$_POST['txtbuscar'].'%';
		$res = $pdo->query("SELECT * from receitas where descricao LIKE '$txtbuscar' or vencimento_fatura LIKE '$txtbuscar' order by id desc");
	...
	//TOTALIZAR OS REGISTROS PARA PAGINAÇÃO
		$res_todos = $pdo->query("SELECT * from receitas");
	...
	for ($i=0; $i < count($dados); $i++) { 
			...
			$id = $dados[$i]['id'];	
			$despesas = $dados[$i]['despesas'];
			$descricao = $dados[$i]['descricao'];
			$tipo_despesa = $dados[$i]['tipo_despesa'];
			$valor = $dados[$i]['valor'];
			$vencimento_fatura = $dados[$i]['vencimento_fatura'];
			$status_pagamento = $dados[$i]['status_pagamento'];
